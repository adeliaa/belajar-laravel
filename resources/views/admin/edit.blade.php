    ...
    <div class="w-full px-6 mx-auto">
        ...
        <form method="POST" action="{{  route('adminProfile', ['id' => $profile->id_user, 'action' => 'update']) }}">
            @csrf
            @method('POST')
                ...
                <div class="flex flex-wrap -mx-3">
                    <div class="flex-none w-auto max-w-full px-3">
                        <div class="text-base ease-soft-in-out h-18.5 w-18.5 relative inline-flex items-center justify-center rounded-xl text-white transition-all duration-200">
                            <img src="{{ asset('storage/img/profile/user/' . $profile->photo) }}" alt="profile_image" class="w-full shadow-soft-sm rounded-xl" />
                        </div>
                    </div>
                            ...
                            <div class="flex-auto p-6">
                                ...
                                <div class="flex flex-wrap -mx-3">
                                    <div class="w-full max-w-full px-3 shrink-0 md:w-full md:flex-0">
                                        <div class="mb-4">
                                            <label for="nik" class="inline-block mb-2 ml-1 font-bold text-sm text-slate-650 dark:text-white/80">NIK</label>
                                            <input type="nik" name="nik" value="{{ old('nik', $profile->nik) }}" class="focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-gray-50 bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" readonly />
                                        </div>
                                    </div>
                                </div>
                                <div class="flex flex-wrap -mx-3">
                                    <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                        ...
                                        <div class="mb-4">
                                            <label for="id_unit" class="inline-block mb-2 ml-1 font-bold text-sm text-slate-650 dark:text-white/80">Unit</label>
                                            <select name="id_unit" id="unit" class="form-control focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" disabled>
                                                @foreach($units as $id_unit => $nama_unit)
                                                <option value="{{ $id_unit }}" {{ $id_unit == old('id_unit', $profile->id_unit) ? 'selected' : '' }}>{{ $nama_unit }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="w-full max-w-full px-3 shrink-0 md:w-6/12 md:flex-0">
                                        ...
                                        <div class="mb-4">
                                            <label for="division" class="inline-block mb-2 ml-1 font-bold text-sm text-slate-650 dark:text-white/80">Divisi</label>
                                            <select name="id_division" id="division" class="form-control focus:shadow-primary-outline dark:bg-slate-850 dark:text-white text-sm leading-5.6 ease block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 outline-none transition-all placeholder:text-gray-500 focus:border-blue-500 focus:outline-none" disabled>
                                                @foreach($divisions as $id_division => $nama_division)
                                                <option value="{{ $id_division }}" {{ $id_division == old('id_division', $profile->id_division) ? 'selected' : '' }}>{{ $nama_division }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    <div class="w-full max-w-full px-3 shrink-0 md:w-full md:flex-0">
                                        <div class="mb-4">
                                            <button type="button" id="edit" class="focus:outline-none text-white bg-blue-600 hover:bg-blue-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-600 dark:focus:ring-red-800">Edit</button>
        ...
        <form action="{{ route('profile.updatePhoto', $profile->id_user) }}" method="POST" enctype="multipart/form-data">
            @csrf
